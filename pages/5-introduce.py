...
> 근대 건축물 데이터는 매우 부족했고, 지자체별 관리가 미흡했습니다.

- 📈 수도권 / 수도권 외 도시의 격차를 줄이는 것에 대해 고려해보자
- 📊 옛 건축물, 축제 등에 대한 데이터를 분석하여 지역 활성화와의 연관성을 확인해보자
...
with cols[1]:
    st.markdown("**최수빈🍖**")
    st.markdown("- 시각화 및 Streamlit 개발")
    st.markdown("- 리서치 및 보고서 작성")

with cols[2]:
    st.markdown("**조효림🥔**")
    st.markdown("- 관광 자원 조사")
    st.markdown("- 모델 설계 기획")

with cols[3]:
    st.markdown("**최하연🍞**")
    st.markdown("- 발표 자료 제작")
    st.markdown("- 지역 트렌드 분석")
